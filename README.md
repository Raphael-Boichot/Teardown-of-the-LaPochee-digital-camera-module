...
Some more details :
- my unit was probably new as the flash memory was empty and the protective screen cover never removed. It looks like a very well finished products in hand. The interface is responsive and quite intuitive to explore after a quick tour.
...
- The options from the capture menu are redunding in the view menu. Basically you can take picture with the after effects and send them or edit them later and send them. Which is cool.
- the device can store 100 images in 96x96 pixels but I have no idea of the grayscale depth due to the absurdly bad quality of the display. It *looks* like non dithered image from what I can see. Technically, the 1MB flash chip can anyway easily store 100 images in 8 bpp. I guess the 100 images limit cover both received images and own stored images.
...
- the picture options are basically similar to a Game Boy Camera with special effects like stretching, negative, zooming, mirrors, binarization, etc. One fun option is to send a mosaic picture so that you are basically impossible to recognize.
